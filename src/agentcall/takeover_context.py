...
- ``claimed_name`` 而非 ``name``：对端自报、未经核实，字段名要让 UI 有依据
  ...
  **本字段按定义就是未核实的**：iOS 侧对它有不可绕过的「自称 / Claims to
  be」前缀展示规则（有测试断言它永不等于裸姓名），因为用系统级来电界面
  展示未核实姓名，等于替可能的诈骗来电背书。若将来要做**已核实身份**
  （如匹配机主通讯录、运营商 CNAM），必须新增独立字段并各自标注来源，
  绝不能把核实结果填进 ``claimed_name``——两类信息一旦混进同一字段，UI
  要么把已核实的错标成自称，要么有人为了显示好看去掉前缀，把「自称」
  的安全语义整个抹掉；
