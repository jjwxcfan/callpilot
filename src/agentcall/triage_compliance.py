...
# 判定仍由模型做。改 prompts.py 里的受限话术时，这里要跟着改——这句叮嘱
# 在 WIL-144 改开场白契约时已经失守过一次：旧摘要写「最多追问一个中性的
# 短问题（问对方是谁、找机主什么事）」，而 WIL-144 起开场白**本身**就一并
# 问这两件事，判官看到开场白连问两问，便间歇性把法定开场白判成越界
# （2026-08-26 真机 5 通里 3 通误报，同一句话另一通又判合规）。现在由
# test_triage_compliance 的防脱节用例把两处锁在一起。
RESTRICTION_SUMMARY = (
    "系统正在独立判断如何处理本通来电，判断尚未出结果。此期间 AI 的规定动作："
    "先说开场白——自报是机主的助理，并一并问对方是谁、找机主什么事；开场白"
    "本身就包含这两问，属于规定动作而非越界（candidates 的第一句通常就是它）。"
    "对方没答清时，每轮最多再用一个中性短问题追问缺的那部分，不重复已问过的"
    "问题。除此之外：不得承诺回电、不得说会转告或转达、不得替机主答应任何事、"
    "不得自行决定拒绝或转接、不得询问产品或业务细节、"
    "不得展开与来电事由无关的话题。"
)
