...
REQUEST_OWNER_TAKEOVER_SPEC: dict[str, Any] = {
    ...
    "function": {
        ...
        # 真机 2026-08-19 spam 演练教训：这里曾写「仅当来电者明确要求找机主本人，
        # 或对话符合偏好时」——「点名找本人」成了独立充分条件，推销话术一句
        # "let me talk to the owner" 就绕过偏好甄别。转不转的判断只归系统提示
        # 里的接管规则，这里不再另立触发条件。
        "description": (
            "请求把当前来电转给机主手机真人接听。仅当按系统提示中的真人接管规则"
            "（含机主偏好及其甄别要求）判断完本通来电、结论是应转接时调用一次；"
            "不要用参数复述偏好、来电内容或模型推理。系统会负责垫话、等待和媒体切换。"
        ),
        "parameters": {"type": "object", "properties": {}, "required": []},
    },
}
